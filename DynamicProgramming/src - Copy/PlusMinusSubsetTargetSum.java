...
/**
 * Given an array and a target sumber find the total number of ways in which we can put + or -
 * for all the array elements such that the expression equals the target sum.
 *
 * This question is equivalent to asking to divide the array into two halves such that their difference is that number.
 * Using the solution of CountPartitionsWithGivenDifference
 */
public class PlusMinusSubsetTargetSum {
    public static void main(String[] args) {
        int[] arr = {1, 2, 3, 2};
        int target = 3;
        int totalSum = Arrays.stream(arr).reduce(0, (a, b) -> a + b);
        if((totalSum + target)%2 == 1){ // it means dividing by 2 will result into a decimal number
            System.out.println("No Partitions possible");
        }

        int requiredSubsetSum = (totalSum + target) / 2;
